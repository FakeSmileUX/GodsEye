...
def exit_message(port = 80):  # Message when GodsEye exit
    choice = input(localization.lang_exit_message["choice"])
    choice.lower()
    if choice == 'r':
        run_command(['sudo', 'python3', 'godseye.py'])
    elif choice == 'm':
...
def terms_of_service_message():  # menu where user select what they wanna use
    # Terms Of Service
   # print("\n\n\n              {1}Nothing {0}Is {2}- {1}Real{0}...      ".format(red, purple, blue))
    
    #if input("\n\n\n\n{2}[{1}!{2}]{3} Do you agree to use this tool for educational/testing purposes only? {1}({0}Y{1}/{2}N{1})\n{2}GodsEye >>> {0}".format(default_palette[2], default_palette[4], default_palette[0], orange)).upper() != 'Y':
    #    run_command('clear')
